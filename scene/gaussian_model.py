...
class GaussianModel:
    ...
    def create_from_xyz(self, xyz, exposure_mapping, spatial_lr_scale):
        self.spatial_lr_scale = spatial_lr_scale

        point_count = xyz.shape[0]

        features = torch.zeros((point_count, 3, (self.max_sh_degree + 1) ** 2)).float().cuda()

        print("Number of points at initialisation : ", point_count)

        dist2 = torch.clamp_min(distCUDA2(xyz), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        rots = torch.zeros((point_count, 4), device="cuda")
        rots[:, 0] = 1

        opacities = self.inverse_opacity_activation(0.1 * torch.ones((point_count, 1), dtype=torch.float, device="cuda"))
        ...
        if exposure_mapping is not None:
            ...
            exposure = torch.eye(3, 4, device="cuda")[None].repeat(len(exposure_mapping), 1, 1)
            self._exposure = nn.Parameter(exposure.requires_grad_(True))

    """Begin: ADC from DashGaussian"""
