...
def compute_photometric_loss(gt_image, image):
...
def compute_gaussian_score_fastgs(camlist, gaussians, pipe, bg, args, DENSIFY = False, gt_image_dict=None, pred_accumulated_dict=None, pos_and_neg=False, inherit_cal=False, move_range=False):
    ...
    for view in range(len(camlist)):
        ...
        gt_image = gt_image_dict[my_viewpoint_cam.uid].squeeze(0)
        if gt_image.device.type == "cpu":
            gt_image = gt_image.to("cuda", non_blocking=True)
        if inherit_cal or pos_and_neg:
            pred_accumulated = pred_accumulated_dict[my_viewpoint_cam.uid].squeeze(0)
            if pred_accumulated.device.type == "cpu":
                pred_accumulated = pred_accumulated.to("cuda", non_blocking=True)
        if inherit_cal:
            render_image = pred_accumulated
        else:
            render_image = render_fastgs(my_viewpoint_cam, gaussians, pipe, bg, args.mult, render_size=gt_image.shape[-2:], pos_and_neg=pos_and_neg)["render"]
            if pos_and_neg:
                previous_accumulated = pred_accumulated
                render_image = render_image + previous_accumulated

        if move_range:
            photometric_loss = compute_photometric_loss((gt_image+1.0)/2.0, (render_image+1.0)/2.0)
        else:
            photometric_loss = compute_photometric_loss(gt_image, render_image)
        ...
        render_pkg = render_fastgs(my_viewpoint_cam, gaussians, pipe, bg, args.mult, get_flag = get_flag, metric_map = metric_map, render_size=gt_image.shape[-2:], pos_and_neg=pos_and_neg)
